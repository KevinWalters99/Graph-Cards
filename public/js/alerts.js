...
var Alerts = {
    refreshTimer: null,
    _prevScores: {},   // { "ABR-ABR": "awayScore-homeScore" } for blink detection
    ...
    loadTicker: function() {
        API.get('/api/alerts/scroll/data').then(function(result) {
            ...
            // Detect score changes from previous refresh
            var newScores = {};
            var changedGames = {};
            for (var i = 0; i < result.items.length; i++) {
                var item = result.items[i];
                if (item.type === 'game' && item.isLive) {
                    var gameKey = (item.away.abbr || '') + '-' + (item.home.abbr || '');
                    var scoreStr = (item.away.score !== null ? item.away.score : '') + '-' + (item.home.score !== null ? item.home.score : '');
                    newScores[gameKey] = scoreStr;
                    if (Alerts._prevScores[gameKey] && Alerts._prevScores[gameKey] !== scoreStr) {
                        changedGames[gameKey] = true;
                    }
                }
            }
            Alerts._prevScores = newScores;
            ...
            for (var i = 0; i < result.items.length; i++) {
                ...
                if (item.type === 'game') {
                    var gameKey = (item.away.abbr || '') + '-' + (item.home.abbr || '');
                    var cls = 'ticker-item ticker-game';
                    if (item.isLive) cls += ' ticker-live';
                    if (changedGames[gameKey]) cls += ' ticker-score-changed';
                    // Game score format: [logo] ABR score - [logo] ABR score (status)
                    itemsHtml += '<span class="' + cls + '">';
