...
class UploadController
{
    ...
    /**
     * POST /api/uploads/general-costs
     */
    public function generalCosts(array $params = []): void
    {
        $userId = Auth::getUserId();
        $secrets = $GLOBALS['cg_secrets'];
        ...
        $originalName = $file['name'];
        $storedName = uniqid('general_costs_', true) . '.csv';
        $storedPath = $secrets['upload']['upload_dir'] . $storedName;
        ...
        try {
            ...
            $parser = new GeneralCostsCsvParser(cg_db(), $userId);
            $result = $parser->parse($storedPath);
