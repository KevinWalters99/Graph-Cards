...
class ParserController
{
    ...
    public function createPlayer(array $params = []): void
    {
        ...
        $pdo = cg_db();
        $stmt = $pdo->prepare(
            "INSERT INTO CG_Players (first_name, last_name, primary_position, current_team_id,
                    minor_league_level, prospect_rank, popularity_score, draft_year, draft_round, draft_pick, draft_status)
             VALUES (:first, :last, :pos, :team, :milb, :rank, :pop, :dy, :dr, :dp, :ds)"
        );
        $stmt->execute([
            ':first' => $firstName,
            ':last'  => $lastName,
